...
def test_disable_customer_blocked_until_mfa_is_set_up() -> None:
    ...
    with pytest.raises(MfaNotConfiguredError):
        disable_customer(actor=manager, customer_id=result.customer.id, reason="fraud")
...
def test_disable_customer_succeeds_once_mfa_is_confirmed() -> None:
    ...
    customer = disable_customer(
        actor=manager, customer_id=result.customer.id, reason="Confirmed fraud report"
    )
    ...
    assert entry.reason == "Confirmed fraud report"
...
def test_disable_customer_denied_for_order_staff_even_with_mfa() -> None:
    ...
    with pytest.raises(InsufficientPermissionError):
        disable_customer(actor=order_staff, customer_id=result.customer.id, reason="fraud")


@pytest.mark.django_db
def test_disable_customer_requires_a_non_empty_reason() -> None:
    manager = cast(Staff, StaffFactory(role=Staff.Role.STORE_MANAGER))
    _confirm_mfa(manager)
    result = register_customer(email="noreason@example.com", password="a-strong-password-123")
